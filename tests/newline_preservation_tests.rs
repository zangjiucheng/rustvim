...
fn test_file_newline_preservation() {
    ...
    let results = editor.load_files(&[file_with_newline.to_str().unwrap().to_string()]);
    ...
    let results2 = editor2.load_files(&[file_without_newline.to_str().unwrap().to_string()]);
    ...
    let _ = editor3.edit_file(new_file.to_str().unwrap()); // This creates a new file
    ...
    editor3
        .buffer_mut()
        .insert_char(rustvim::buffer::Position::new(0, 0), 'h');
    editor3
        .buffer_mut()
        .insert_char(rustvim::buffer::Position::new(0, 1), 'i');
...
fn test_empty_file_handling() {
    ...
    let mut editor = Editor::new();
    let results = editor.load_files(&[empty_file.to_str().unwrap().to_string()]);
    assert!(results[0].is_ok());
...
fn test_single_newline_file() {
    ...
    let mut editor = Editor::new();
    let results = editor.load_files(&[newline_file.to_str().unwrap().to_string()]);
    assert!(results[0].is_ok());
    ...
    let saved_newline =
        fs::read_to_string(&output_newline).expect("Failed to read saved newline file");
