use rustvim::commands::{EditCommand, ModeSwitchCommand, MovementCommand};
use rustvim::editor::Mode;
use rustvim::input::Key;
use rustvim::keymap::{
    Action, Keymap, KeymapConfig, KeymapConfigBuilder, KeymapProcessor, PendingAction,
};
...
mod keymap_tests {
    ...
    fn test_keymap_bind_unbind() {
        ...
        keymap.bind(
            Mode::Normal,
            Key::Char('x'),
            Action::Edit(EditCommand::DeleteChar),
        );
    ...
    fn test_keymap_mode_isolation() {
        ...
        if let Some(Action::Edit(EditCommand::DeleteChar)) =
            keymap.lookup(Mode::Normal, &Key::Char('x'))
        {
        ...
        if let Some(Action::Move(MovementCommand::Left)) =
            keymap.lookup(Mode::Insert, &Key::Char('x'))
        {
    ...
    fn test_get_bound_keys() {
        ...
        keymap.bind(
            Mode::Normal,
            Key::Char('j'),
            Action::Move(MovementCommand::Down),
        );
